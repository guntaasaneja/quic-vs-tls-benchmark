...
async def perform_http_request(
   ...
   # Measure handshake latency (approx)
    handshake_start = time.time()
    await client.wait_connected()  # wait until QUIC handshake completes
    handshake_end = time.time()
    handshake_time = handshake_end - handshake_start

    # Measure request-response RTT (approx)
    request_start = time.time()
    # your GET or POST request already happens here
    request_end = time.time()
    rtt = request_end - request_start

    # Compute throughput
    throughput_mbps = (octets * 8) / (elapsed) / 1_000_000  # octets already counted

    logger.info(
        f"Performance: Handshake={handshake_time:.4f}s, "
        f"RTT={rtt:.4f}s, Elapsed={elapsed:.4f}s, "
        f"Size={octets} bytes, Throughput={throughput_mbps:.3f} Mbps"
    )
...
async def main(
    ...
    async with connect(
        ...
        if parsed.scheme == "wss":
            ...
            size = 1024  # Start from 1 KB
            MAX_SIZE = 1024 * 1024 * 1024  # 1 GB

            while size <= MAX_SIZE:
                data = os.urandom(size)  # generate random data of 'size' bytes
                start = time.time()
                ...
                elapsed = time.time() - start
                throughput = (size * 8) / (elapsed * 1_000_000) if elapsed > 0 else 0.0
                print(f"{size/1024:10.0f} | {elapsed:10.4f} | {throughput:18.3f}")

                size *= 2  # double data size each round

            # ✅ Only close once after loop finishes
            await client.close(error_code=ErrorCode.H3_NO_ERROR)
